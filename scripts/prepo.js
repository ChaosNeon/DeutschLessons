...
let prepos = ['bis', 'für', 'ohne', 'durch', 'um', 'gegen', 'an', 'in', 'über', 'auf', 'unter', 'vor', 'hinter',
'neben', 'zwischen', 'von', 'aus', 'nach', 'zu', 'mit', 'bei', 'ab', 'seit', 'ausserchen', 'gegenüber'];
//----------------------------------------------------------------------------------------------------------------------//
...
function startGame() {
    let numLet = Math.floor(Math.random() * 25);
    if(prepos[numLet] === label.textContent){  //If the letter is the same, the function is called again
        nextQuestion();
    } else {
        label.textContent = prepos[numLet];
    }
